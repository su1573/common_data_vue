<template>
  <div>
    ...
    >
        ...
        <el-table-column prop="ivstmngcode" :label="$t('AppMain.Ivstmngcode')" width="180" />
        <el-table-column prop="ivstmngname" :label="$t('AppMain.Ivstmngname')" width="180" />
        ...
        <el-table-column prop="ivstmanagerid" :label="$t('AppMain.Ivstmanagerid')" width="180" />
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="$t('global.Show')+$t('global.Details')">
      <el-form :model="form" :inline="true" size="mini" :disabled="!isEdit">
                <el-form-item :label-width="formLabelWidth" :label="$t('AppMain.Ivstmngcode')">
          <el-input v-model="form.ivstmngcode" autocomplete="off" />
        </el-form-item>
                <el-form-item :label-width="formLabelWidth" :label="$t('AppMain.Ivstmngname')">
          <el-input v-model="form.ivstmngname" autocomplete="off" />
        </el-form-item>
          ...
          <el-input v-model="form.cifid" autocomplete="off" />
                ...
                <el-form-item :label-width="formLabelWidth" :label="$t('AppMain.Ivstmanagerid')">
          <el-input v-model="form.ivstmanagerid" autocomplete="off" />
        </el-form-item>
              </el-form> 
...
import ivstmanagerApi from "@/api/biz/manager/ivstmanager/ivstmanager.js";
...
          arr.limit = pageSize;
      ivstmanagerApi.list(arr, result => {
    ...
    update() {
      if (this.isEdit) {
        ...
        ivstmanagerApi.update(arr, result => {
          this.$message({
            type: "success",
    ...
    deleteClick(row) {
      this.$confirm(this.$t('global.ConfirmDelete'), this.$t('global.Warn'), {
        ...
        .then(() => {
          // 请求后台，成功后调用
          var arr = {
            ivstmngcode: row.ivstmngcode
          };
          ivstmanagerApi.remove(arr, result => {
            if (result.status === 200) {
              this.$message({
                type: "success",
